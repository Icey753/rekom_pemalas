...
def crawl_executive_summary_by_dosen(
    pdf_path: str = "Executive Summary - DIV Komputasi Statistik.pdf",
    output_json: str = "hasil_executive_summary_dosen.json",
    output_csv: str = "hasil_executive_summary_dosen.csv",
    start_page: int = 18,
    end_page: int = 32  # Halaman 18-32 adalah daftar isi berdasarkan nama Dosen / Pembimbing
):
    """
    Mengekstrak dosen (pembimbing), mahasiswa (penulis), judul, dan rentang halaman dokumen
    dari daftar isi Executive Summary bagian Dosen/Pembimbing (Halaman 18 sampai 32) menggunakan pdfplumber.
    """
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"File PDF '{pdf_path}' tidak ditemukan.")

    print(f"Membuka file: {pdf_path}")
    print(f"Mengekstrak daftar isi berdasarkan Dosen dari halaman {start_page} hingga {end_page}...")
    ...
    for page_num, line in full_lines:
        # Format pada bagian ini adalah: NAMA DOSEN & NAMA MAHASISWA
    ...
    for idx, item in enumerate(raw_entries, 1):
        parts = item["raw_dosen_mahasiswa"].split(" & ")
        dosen = parts[0].strip()
        penulis = parts[1].strip() if len(parts) > 1 else ""
    ...
    print(f"Berhasil mengekstrak {len(extracted_data)} entri skripsi berdasarkan Dosen!")
...
if __name__ == "__main__":
    data = crawl_executive_summary_by_dosen()
    print("\n--- Ringkasan Data ---")
    print(f"Total skripsi: {len(data)}")
    if data:
        print(f"Contoh data pertama : [Dosen: {data[0]['dosen']}] [Mhs: {data[0]['penulis']}] - {data[0]['judul'][:60]}...")
        print(f"Contoh data terakhir: [Dosen: {data[-1]['dosen']}] [Mhs: {data[-1]['penulis']}] - {data[-1]['judul'][:60]}...")
